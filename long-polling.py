    ...
    while True:
        ...
        for update in updates.get("result", []):
            ...
            if text == "/start":
                ...
                send_message(chat_id, get_devians_details(roll_no), message_id)
                ...
                inpu = text.split()
                if len(inpu) == 2:
                    response = get_github_profile(inpu[1])
                elif len(inpu) == 3 and inpu[1] == "repo":
                    response = get_github_repo(inpu[2])
                else:
                    response = "ℹ Usage: /github <username> or /github repo <username>/<repo>"
                send_message(chat_id, response, message_id)

            elif text == "/joke":
                send_message(chat_id, get_joke(), message_id)
